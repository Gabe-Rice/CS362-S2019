...
void adventurerFunc(struct gameState *state, int currentPlayer)
{
	int drawntreasure=0;
	//int currentPlayer = whoseTurn(state);
	int cardDrawn;
...
int mineFunc(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2)
{
  int i, j;
...
void tributeFunc(struct gameState *state, int nextPlayer, int currentPlayer)
{
  int i;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
      case adventurer:
		    adventurerFunc(state, currentPlayer);
		    return 0;
 /*   while(drawntreasure<2){
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
	  shuffle(currentPlayer, state);
    ...
    case mine:
      mineFunc(state, currentPlayer, handPos, choice1, choice2);
...
/*      for (i = 0; i < 3; i++)
    ...
    case minion:
      minionFunc(state, handPos, currentPlayer, choice1, choice2);
      return 0;
      //+1 action
/*      state->numActions++;
    ...
    case tribute:
      tributeFunc(state, nextPlayer, currentPlayer);
      return 0;
/*      if ((state->discardCount[nextPlayer] + state->deckCount[nextPlayer]) <= 1){
	if (state->deckCount[nextPlayer] > 0){
	  tributeRevealedCards[0] = state->deck[nextPlayer][state->deckCount[nextPlayer]-1];
